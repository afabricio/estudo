...
public class GenericDAO {
	...
	public <T> List<T> findByNamedQuery(String namedQuery) {
		return this.em.createNativeQuery(namedQuery).getResultList();
	}
	...
	@SuppressWarnings("unchecked")
	public <T> List<T> findByNamedQuery(String namedQuery,
			QueryParameters queryParameter, int resultLimit) {
		Set<Entry<String, Parameter>> rawParameters = queryParameter
	...
	public <T> T executeFunction(String funcao, QueryParameters queryParameter, Class<T> returnType) {
		...
		String queryText = String.format("SELECT %s( %s ) FROM DUAL ", funcao,
				pramBuilder.toString());
	 ...
	 * @return
	 */
	public OutMap executeProcedure(String procName,
			QueryParameters queryParameter) {
		Set<Entry<String, Parameter>> inParameters = queryParameter
				.getParameters().entrySet();
		Set<Entry<String, Parameter>> outParameters = queryParameter
				.getOutParameters().entrySet();

		StoredProcedureQuery proc = em.createStoredProcedureQuery(String
				.format("{ call %s}", procName));

		for (Entry<String, Parameter> entry : inParameters) {
			Parameter param = entry.getValue();

			if (!queryParameter.getOutParameters().containsKey(entry.getKey())) {
				proc.registerStoredProcedureParameter(entry.getKey(),
						param.getClazz(), ParameterMode.IN);

			} else {
				proc.registerStoredProcedureParameter(entry.getKey(),
						param.getClazz(), ParameterMode.INOUT);
			}
		...
		for (Entry<String, Parameter> entry : outParameters) {
			...
			if (!queryParameter.getParameters().containsKey(entry.getKey())) {
				...
				proc.registerStoredProcedureParameter(entry.getKey(),
						outParam.getClazz(), ParameterMode.OUT);
			}
		}
	 ...
	 * @return
	 */
	public StoredProcedureQuery registerProcedure(String procName,
			QueryParameters queryParameter) {
		Set<Entry<String, Parameter>> inParameters = queryParameter
				.getParameters().entrySet();
		Set<Entry<String, Parameter>> outParameters = queryParameter
		...
		StoredProcedureQuery register = em.createStoredProcedureQuery(String
				.format("{ call %s}", procName));

		for (Entry<String, Parameter> entry : inParameters) {
			Parameter param = entry.getValue();
			if (!queryParameter.getOutParameters().containsKey(entry.getKey())) {
				register.registerStoredProcedureParameter(entry.getKey(),
						param.getClazz(), ParameterMode.IN);
	...
	/**
	 * executa procedure utilizando StoredProcedureQuery registrada
	 * anteriormente.
	 * 
	 * @param proc
	 * @param queryParameter
	 * @return
	 */
	public OutMap executeProcedure(StoredProcedureQuery proc,
			QueryParameters queryParameter) {
		Set<Entry<String, Parameter>> inParameters = queryParameter
				.getParameters().entrySet();
		Set<Entry<String, Parameter>> outParameters = queryParameter
	 ...
	 * @return
	 */
	public Map<String, Object> executeProcedureByNamedProcedure(
			String procName, QueryParameters queryParameter) {
		Set<Entry<String, Parameter>> inParameters = queryParameter
				.getParameters().entrySet();
		Set<Entry<String, Parameter>> outParameters = queryParameter
				.getOutParameters().entrySet();
		...
		for (Entry<String, Parameter> entry : outParameters) {
			outs.put(entry.getKey(),
					proc.getOutputParameterValue(entry.getKey()));
		}
